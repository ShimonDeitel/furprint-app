...
struct ContentView: View {
    ...
    @State private var editingEntry: AdoptionEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No anniversary entries yet")
                .font(Theme.bodyFont)
                .foregroundStyle(Theme.textSecondary)
        }
    }

    private func entryRow(_ entry: AdoptionEntry) -> some View {
...
struct EntryEditView: View {
    ...
    var entry: AdoptionEntry?
    var onSave: (AdoptionEntry) -> Void
        ...
        NavigationStack {
            Form {
                Section("Anniversary") {
                    ...
                    TextField("Storynote", text: $f2Text)
            ...
            .navigationTitle(entry == nil ? "Add Anniversary" : "Edit Anniversary")
            .toolbar {
            ...
            .onAppear {
                if let entry {
                    ...
                    f2Text = String(describing: entry.storyNote)
                }
            }
        }
    }
    ...
    private func save() {
        ...
        let result = AdoptionEntry(id: entry?.id ?? UUID(), date: entry?.date ?? Date(), petName: f1Value, storyNote: f2Value)
        onSave(result)
        dismiss()
    }
}
